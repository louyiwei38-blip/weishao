...
**PRD v2.2+** · BTC/USDT 5m · Martingale 4-loss stop · CLOB V2 · Chainlink 结算 · GTC 限价

Polymarket BTC 5 分钟涨跌盘口自动交易机器人：CCXT K 线产生反转信号，CLOB 限价/市价下单，Chainlink oracle 结算，马丁格尔管理仓位。
...
- 信号产生后，交易**当前刚开盘**的 5m 盘口（`btc-updown-5m-{windowStartUnix}`）。
...
└── utils/                           # logger, retry, websocket, telegram…
docs/
└── ARCHITECTURE.md                  # 架构与运行逻辑（推荐阅读）
logs/
├── signals.jsonl
├── trades.jsonl
├── settlements.jsonl                # Chainlink 结算记录
...
```
每 5m → CCXT K 线 → S1/S2 信号 → CLOB 下单
                              ├─ 成交 → pending-bet → Chainlink 结算 → 马丁
