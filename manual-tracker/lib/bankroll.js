...
export function computeStake(state, opts = {}) {
...
export function recordTrade(state, won, meta = {}) {
  const settings = cfg(state);
...
export function resetBankroll(balance, prev = defaultState()) {
...
export function patchSettings(state, settingsPatch) {
  const st = normalizeState(state);
...
export function undoLastTrade(state) {
  const st = normalizeState(state);
