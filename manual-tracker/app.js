...
function render() {
  ...
  document.getElementById('nextStake').textContent = fmt(stake.stakeUsd);
  document.getElementById('nextProfit').textContent = stake.profitUsd != null ? `赢 +${fmt(stake.profitUsd).slice(1)}` : '';
  document.getElementById('stakeMode').textContent =
    stake.mode === 'catch_up'
  ...
  document.getElementById('statWinRate').textContent = fmtPct(s.winRatePct);
  document.getElementById('statWL').textContent = `${s.wins}胜 ${s.losses}负`;
  document.getElementById('statNet').textContent = s.netCount >= 0 ? `+${s.netCount}` : String(s.netCount);
  document.getElementById('statNet').className = 'stat-val ' + (s.netCount >= 0 ? 'pos' : 'neg');
  document.getElementById('statBalance').textContent = fmt(s.balance);
  document.getElementById('statPrincipal').textContent = fmt(s.principal);
  document.getElementById('statTarget').textContent = fmt(s.targetBalance);
  document.getElementById('statGap').textContent = s.gapUsd != null && s.gapUsd > 0 ? fmt(s.gapUsd) : '—';
  ...
  document.getElementById('syncStatus').textContent = apiBase ? `云端: ${apiBase}` : '仅本机 localStorage';
  document.getElementById('setDefaultBet').value = s.settings.defaultBet;
  document.getElementById('setStep').value = s.settings.stepUsd;
  document.getElementById('setStakeMax').value = s.settings.stakeMaxUsd;
  document.getElementById('setTCap').value = s.settings.catchUpProfitCapUsd;
  document.getElementById('setOdds').value = s.settings.profitRatio;
  document.getElementById('setBalance').value = s.balance ?? '';
  document.getElementById('setApi').value = apiBase;
}
...
async function onSaveSettings(e) {
  ...
  state = patchSettings(state, {
    defaultBet: Number(document.getElementById('setDefaultBet').value),
    stepUsd: Number(document.getElementById('setStep').value),
    stakeMaxUsd: Number(document.getElementById('setStakeMax').value),
    catchUpProfitCapUsd: Number(document.getElementById('setTCap').value),
    profitRatio: Number(document.getElementById('setOdds').value),
  });
  const bal = document.getElementById('setBalance').value;
  if (bal !== '' && Number.isFinite(Number(bal))) {
    state = setBalance(state, Number(bal));
  }
  apiBase = (document.getElementById('setApi').value || '').trim();
